...
export const useBitrix24 = () => {
  ...
  const getPointsDone = async (b24Instance) => {
    let f = {"@stageId": ['DT139_65:SUCCESS', 'DT139_65:FAIL']}
    return await getGeoObjects(b24Instance, POINT_TYPE_ID, f)
  }
  ...
  const getItem = async (b24Instance, itemId) => {
    try {
      const result = await b24Instance.callMethod('crm.item.get', {
        entityTypeId: SMART_PROCESS_ID,
        id: itemId
      })
      return result._data?.result?.item || null
    } catch (error) {
